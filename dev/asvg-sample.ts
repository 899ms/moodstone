import { identityFromName, MOOD_KEYS, CUT_KEYS } from '../src/core';
import { renderAnimatedAvatarSvg } from '../src/svg';
const tiles = MOOD_KEYS.map((mood, i) => {
  const spec = { ...identityFromName('Nova' + i), cut: CUT_KEYS[i % CUT_KEYS.length], mood, eyeColor: i % 4 === 3 ? '#111111' : '#FFFFFF' };
